...
class Player {
  ...
  setAudioEvents() {
    this.audio.addEventListener('progress', this.handlePreloaded)
  ...
  handlePreloaded() {
    ...
    if (this.audio) {
      for (let i = 0; i < this.audio.buffered.length; i += 1) {
        ranges.push([
    ...
    const percentage = (
      (100 / this.audio.duration) * (ranges[ranges.length - 1][1])
    ).toFixed(2)

    this.progress.updatePreloader(percentage)
  }
